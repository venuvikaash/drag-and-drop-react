...
import { Tabs, Tab, Box, Avatar, Typography, IconButton } from "@mui/material";
...
const BirthdayTabs = () => {
  ...
  return (
    <Box className="w-full !h-full p-4 border-[1px] border-[#DFDFDF] bg-white rounded-xl overflow-auto">
      ...
      {tabIndex === 0 && (
        <Box className="mt-4">
          ...
          {birthdaysToday.map((person, idx) => (
            <Box
              key={idx}
              className="mb-6 my-2 bg-[#C1EDEC] rounded-lg px-4 py-2 flex justify-between items-center"
            >
      ...
      {tabIndex === 2 && (
        <Box className="mt-4 text-sm text-gray-500">No New Joinees today.</Box>
      )}
    </Box>
  );
};
